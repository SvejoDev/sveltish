<script lang="ts">
  ...
  import { get } from 'svelte/store'; // Importera get från Svelte's store för att hämta synkrona värden
  ...
  async function confirmUsername() {
    ...
    const $user = get(user); // Hämta synkront värdet från user store

    if (!$user) return; // Om ingen användare är inloggad, gör inget
    ...
    username = ""; // Återställ användarnamn
    isAvailable = false;
  }
